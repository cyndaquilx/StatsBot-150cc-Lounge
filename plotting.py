...
async def create_plot(mmrhistory, season=''):
    ...
    def plot():
        ...
        matplotlib.style.use('lounge_style.mplstyle')
        ax = fig.subplots()
        
        ax.plot(mmrhistory, color='snow', linewidth='1.0')
        xmin, xmax, ymin, ymax = ax.axis()
        ax.set_ylabel("MMR")
        ax.grid(True, 'both', 'both', color='snow', linestyle=':')
        ...
        for i in range(len(ranks)):
            ...
            if season >= 5:
                ...
                ax.hlines(divide, xmin, xmax, colors='snow', linestyle='solid', linewidth=1)
...
async def create_overall_histogram(mmr_list, season):
    ...
    def plot():
        ...
        ax = fig.subplots()
        ax.set_title(f"MMR Distribution (Events Played > 0)", loc="left", fontsize=12)
        ax.set_xlabel("MMR", fontsize=10)
        ax.set_ylabel("Players", fontsize=10)
        ax.grid(True)
        ax.tick_params(labelsize = 10)

        n, bins, patches = ax.hist(mmr_list, alpha=0.5, bins=160, range=(0, 16000), histtype="bar", rwidth = 0.5) #(8) ヒストグラムの描画
        y_max = int(max(n)) + 1
        ax.set_yticks(np.arange(0, y_max, 20))
        ...
        fig.clf()
        return n
    n = await asyncio.to_thread(plot)
    return n, b
